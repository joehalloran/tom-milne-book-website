			    ...
			    <div class="row bookshelf">
			   		...
			   		if ($saved_values) {
				   		foreach ( $saved_values as $post_id ) : 
				   			$post = get_post($post_id); ?>
				   			<div class="col-sm-3 col-xs-6 book-preview" data-title="<?php echo esc_attr($post->post_title); ?>" data-author="<?php echo esc_attr(get_post_meta($post_id , 'book_author_surname', true)); ?>">
						        ...
						        if ($all_user_comments) {
							        ...
							        foreach ( $all_user_comments as $comment ) {
							        	if ( $i == 0) {
							        		echo '<h3 style="display: none;">My Review</h3>';
							        		echo '<p style="display: none;" class="excerpt">'.$comment->comment_content.'</p>';
							        		
							        	} elseif ( $i == 1 ) {
							        		echo '<h3 style="display: none;">My other comments</h3>';
							        		echo '<p style="display: none; color: #337ab7;" class="excerpt"><strong><span>'.$i.'. </span>Comment on: </strong><em>'.$comment->comment_date.'</em></p>';
								        	echo '<p style="display: none;" class="excerpt">'.$comment->comment_content.'</p>'; 
							        	} else {
								        	echo '<p style="display: none; color: #337ab7;" class="excerpt"><strong><span>'.$i.'. </span>Comment on: </strong><em>'.$comment->comment_date.'</em></p>';
