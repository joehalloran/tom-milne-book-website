...
function rosendalereads_scripts() {
	...
	// Enqueue custom css:
	wp_enqueue_script( 'rosendale-reads-js',  get_template_directory_uri() . '/js/rosendalereads.js', array( 'jquery' ), '1.0.1', true );
...
function get_genre($post_id) {
	...
	foreach($genres as $genre) {
		...
		if ($parent == get_cat_ID( "Genre" )) { //if a Genre category
			...
			echo '<a href="'. esc_url( get_category_link( $genre->term_id ) ).'"' .' rel="category tag">'.esc_html($genreName)."</a>, ";
		} //endif
	}//end genre
...
function reads_get_star_rating( $post_id ) {
	...
	if ( $starCount && $starValue ) {
		...
		$averageRating = round($averageRating, 0, PHP_ROUND_HALF_UP);
