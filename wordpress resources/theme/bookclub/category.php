	   		...
	   		<div class="col-md-8 my-bookshelf-main-section">
			    ...
			    <div class="row bookshelf">
			   		...
			   		if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
			   			<div class="col-sm-12 book-preview" data-title="<?php esc_attr(the_title(), 'bookclub'); ?>" data-author="<?php echo esc_attr(get_post_meta($post->ID, 'book_author_surname', true)); ?>" data-rating="<?php echo esc_attr(reads_get_star_rating($post->ID)); ?>">
				   			<a href="<?php the_permalink(); ?>">
				   				<?php the_post_thumbnail('full', array( 'class' => 'img-rounded book-thumbnail book-thumbnail-list')); ?>
					        	<h3><?php the_title(); ?><br /><small><?php $authorName = get_rbc_authorName($post->ID); echo esc_html($authorName);?></small></h3>
					        </a>
					        <p class="excerpt"><?php echo get_the_excerpt();?><br />
					        <a href="<?php the_permalink(); ?>"><?php _e('More', 'bookclub') ?></a></p>
					        <?php age_recommended_images($post->ID); ?>
					        <?php 
