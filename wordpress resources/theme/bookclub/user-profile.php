...
// Display "My Bookshelf", "My Top 5", etc.. if I own the page or "Joe's bookshelf" if I am a visitor 
function decide_display_name( $current_user, $iHaveAccess ) {
	if ( $iHaveAccess ) {
		return "My";
	} else {
		return $current_user->first_name."'s";
	}
}
...
$displayName = decide_display_name( $current_user, $iHaveAccess );
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	foreach ($topFive as $book) {
		if ( is_string($book) && ( mb_strlen($book) < 80 ) ) {
			// echo "fine"; 
		} elseif ( $book == "" ) {
			// echo "fine";
		} else {
	...
	<div class="container">
  		<div class="row">   
	        <div class="col-md-12">
				...
				if ( $group = Groups_Group::read_by_name( 'Teachers' ) ) {
					$is_a_teacher = Groups_User_Group::read( $userid , $group->group_id );
				}
				if ($is_a_teacher) {
	        		echo __('Welcome to', 'bookclub').' '.( $iHaveAccess ? 'your' : $user->first_name." ".$user->last_name."'s")." bookshelf"; ?></h1>
	        	<?php
	        	} else {
	        		echo __('Welcome to', 'bookclub').' '.( $iHaveAccess ? 'your' : $user->first_name."'s")." bookshelf"; ?></h1>
		        <?php 
			    }
		        if ( $saved_values ) {
		        	$booksRead = count($saved_values);
		        	if ( $iHaveAccess ) {
			        	printf( esc_html( _n('You have read %s book.', 'You have read %s books.', $booksRead, 'bookclub') ), $booksRead);
					} else {	
						printf( esc_html( _n( $user->first_name.' has read %s book.', $user->first_name.' has read %s books.', $booksRead, 'bookclub') ), $booksRead);
					}
				} elseif ( $iHaveAccess ) {
					_e('You have not got any books on your bookshelf. Write a review of a book you have read. When your teacher checks your review, the book will appear here.', 'bookclub');
				} else {
					_e( 'This person has not reviewed any books yet', 'bookclub'); 
				}	
		        ?>
		        <hr />
	        </div><!-- /.col 12 -->
				   		...
				   		foreach ( $saved_values as $post_id ) : 
				   			...
				   			<div class="col-sm-3 col-xs-6 book-preview" data-title="<?php echo esc_attr($post->post_title); ?>" data-author="<?php echo esc_attr(get_post_meta($post_id , 'book_author_surname', true)); ?>" data-rating="<?php echo esc_attr(reads_get_star_rating(get_the_ID())); ?>">
					   			<a href="<?php echo post_permalink($post_id); ?>">
						        	...
						        	<h2 style="display: none;"><?php echo $post->post_title; ?><br /><small><?php $authorName = get_rbc_authorName($post_id); echo esc_html($authorName);?></small></h2>
						        </a>
						        <?php
						        ...
						        if ($all_user_comments) {
							        ...
							        foreach ( $all_user_comments as $comment ) {
							        	if ( $i == 0) {
							        		echo '<h3 style="display: none;">'.( $iHaveAccess ? __('My','bookclub') : $user->first_name."'s").__(' review','bookclub').'</h3>';
							        		echo '<p style="display: none;" class="excerpt">'.$comment->comment_content.'</p>';
							        		?>
							        		<div id="user_profile_rating" class="row" style="display: none;">
							        			<div class="col-md-12">
											        <h3><?php echo ( $iHaveAccess ? __('My','bookclub') : $user->first_name."'s")?> rating</h3>
											        <?php
											        //echo do_shortcode( '[user_star_rating themes="user" id="'.$post_id .'" user="'.strval($userid).'"]' );
											        echo show_user_star_rating($post_id, $userid);
											        ?>
										        </div>
									        </div>
									        <?php
							        	} elseif ( $i == 1 ) {
							        		echo '<h3 style="display: none;">'.( $iHaveAccess ? __('My','bookclub') : $user->first_name."'s").__(' other comments', 'bookclub').'</h3>';
						        ...
						        ?>
					        </div><!-- /.col .book-preview -->
			        	<?php endforeach;
			        } ?>
		        </div> <!-- /.row .bookshelf -->
		        <div class="row">
			        <div class="col-md-12">
			        	<h2>Other favourite books</h2>
			        	<p>Tell us about books that should be included in our top 100, but aren't.</p>
			        	<p><a href="<?php echo home_url( '/' )."other-favourite-books" ?>">  Add your suggestions here</a></p>
			        </div>
			    </div><!-- /.row -->
		    ...
		    <div class="col-md-4 my-bookshelf-sidebar">
		    	<div class="panel panel-default">
                    <div class="panel-heading" id="top-five-books">
                        <h3><?php $top5Title = ( $iHaveAccess ? 'My ' : $user->first_name."'s " ).'Top 5' ; 
                        _e( $top5Title , 'bookclub'); ?></h3>
                    </div>
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <div class="btn-group" role="group" aria-label="...">
                            <button id="listView" type="button" class="btn btn-default"><?php _e('List', 'bookclub'); ?></button>
                            <button id="iconView" type="button" class="btn btn-default btn-red"><?php _e('Icons', 'bookclub'); ?></button>
                        </div>
                    </div>
                </div>
