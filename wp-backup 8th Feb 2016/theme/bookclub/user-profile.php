			    ...
			    <div class="row bookshelf">
			   		...
			   		if ($saved_values) {
				   		foreach ( $saved_values as $post_id ) : 
				   			$post = get_post($post_id); ?>
				   			<div class="col-sm-3 book-preview" data-title="<?php echo esc_attr($post->post_title); ?>" data-author="<?php echo esc_attr(get_post_meta($post_id , 'book_author_surname', true)); ?>">
					   			<a href="<?php echo post_permalink($post_id); ?>">
					   				<?php echo get_the_post_thumbnail($post_id, 'full', array( 'class' => 'img-responsive img-rounded book-thumbnail', 'data-id' => esc_attr($post_id), 'data-title' => esc_attr($post->post_title ))); ?>
						        	<h3 style="display: none;"><?php echo $post->post_title; ?><br /><small><?php $authorName = get_post_meta($post_id, 'book_author_firstname', true) . ' ' . get_post_meta($post_id, 'book_author_surname', true); echo esc_html($authorName);?></small></h3>
						        </a>
						        <p class="excerpt" style="display: none;"><?php echo get_excerpt_by_id($post_id); ?><br />
						        <a href="<?php echo post_permalink($post_id); ?>"><?php _e('More', 'bookclub'); ?></a></p>
					        </div><!-- /.book-preview -->
			        	<?php endforeach;
			        } ?>
			    </div><!-- /.row -->
