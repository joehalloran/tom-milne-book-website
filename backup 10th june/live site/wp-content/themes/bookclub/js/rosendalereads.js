jQuery(document).ready(function($) {
	...
	$('#listView').on('click', function() { 
		if ( !$( this ).hasClass( "disabled" ) && !$( this ).hasClass( "btn-red" ) ) {
			$(this).addClass('btn-red').siblings().removeClass('btn-red');
			$(this).parent().siblings('.btn-group').children('button').addClass('disabled');
			$books.each( function() {
	...
	$('#iconView').on('click', function() {
			 {
			$(this).addClass('btn-red').siblings().removeClass('btn-red');
			$(this).parent().siblings('.btn-group').children('button').removeClass('disabled');
			$books.each( function() {
	...
	$('#bookShelfOn').on('click', function() {
		if ( !$( this ).hasClass( "disabled" ) && !$( this ).hasClass( "btn-red" ) ) { 
			...
			var $bookCount = ($books.length % 4);
			console.log($bookCount);
			if ($bookCount == 1 ) {
				var i = 0;
				while (i < 11) {
					$('.bookshelf').append('<div class="col-sm-3 col-xs-6 empty-bookshelf"></div>');
					i++;
				}
			} else if ($bookCount == 2 ) {
				var i = 0;
	...
	$('#titleSort').on('click', function(e) {
		...
		if ($(this).is('.ascending') || $(this).is('.descending')) {
			...
			$('.bookshelf').prepend(booksCache.reverse());
		} else {
